...
public class MinMovesFloorTests
{
    ...
    [System.Serializable] private class PuzzleEntry { public string startWord; public string endWord; public string[] solution; public int optimalSteps; }
    ...
    // Daily rework — every DAILY must have optimalSteps >= 4 with an HONEST par (the stored
    // optimalSteps equals the true full-dictionary shortest), and the pool never shrinks.
    // Regenerated by Tools/daily_floor_fix.py (FLOOR=4); independently checked by verify_data.py.
    [Test]
    public void DailyPuzzles_FourStepFloor_HonestPar_PoolMaintained()
    {
        var daily = JsonUtility.FromJson<DailyDefs>(Resources.Load<TextAsset>("Data/daily_puzzles").text);
        Assert.GreaterOrEqual(daily.puzzles.Length, 450, "the daily pool must never shrink below the original 450");

        var failures = new List<string>();
        foreach (var p in daily.puzzles)
        {
            if (p.optimalSteps < 4)
                failures.Add($"daily {p.startWord}->{p.endWord}: optimalSteps {p.optimalSteps} < 4");
            if (p.solution == null || p.solution.Length - 1 != p.optimalSteps)
                failures.Add($"daily {p.startWord}->{p.endWord}: solution length != optimalSteps");
            int trueDist = TrueShortest(p.startWord, p.endWord);
            if (trueDist != p.optimalSteps)
                failures.Add($"daily {p.startWord}->{p.endWord}: stored par {p.optimalSteps} != true shortest {trueDist} (dishonest par)");
        }
        Assert.IsEmpty(failures,
            $"{failures.Count} dailies violate the 4-step floor / honest par:\n" + string.Join("\n", failures.Take(20)));
    }
}
