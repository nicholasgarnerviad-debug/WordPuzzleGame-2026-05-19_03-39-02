...
<p align="center">
  <img src="docs/screenshots/menu-hero.png" width="300" alt="Star Ladder main menu — pixel-space purple backdrop, the gradient Star Ladder logotype, a coin pill, DAILY as a filled orchid-gradient hero, icon-carrying outlined mode buttons, and ghost Library/Stats links">
...
| **Puzzle Library → Tier Select** (level 1) — the entry to Puzzle Show: **7 tiers** as tall, rounded **glowing-outline rows** (full-height — the rect height is set explicitly because the root layout group runs `childControlHeight=false`, which ignores `LayoutElement`) under the runtime **PUZZLE SHOW / Pick a tier** header (the unthemed scene masthead is retired in both views; **HOME is a ghost**). Each row shows its theme (e.g. "3-letter words"), progress (`X/100`) and lock state — the **active tier glows hero-bright**, locked tiers are dim with a **"Clear N in Tier M to unlock"** hint (the old `□` padlock glyph was tofu in Rungo and is gone). Tap an unlocked tier to open its grid. | <img src="docs/screenshots/puzzle-library.png" width="250"> |
| **Puzzle Library → Tier Grid** (level 2, Task 48) — the selected tier's **100** puzzle cards under a header with an **animated tier progress bar**, a ghost **‹ Back** to tier-select, and the **"up next"** card carrying the ONE hero glow + a ▸ play cue (only the active tier renders, for performance). Cards reflect saved progress: **Completed** (**aqua** + ✓ — Direction B retired green), **Unplayed** (surface grey), **Locked** (dim). Tapping an **unbeaten** card launches that exact puzzle; tapping a **beaten** one opens the spoiler-free **Path View** — your best solve + the optimal path uncovered so far ([§7](#7-puzzle-show-tiers)). | <img src="docs/screenshots/puzzle-library-tier.png" width="250"> |
| **Stats** (redesigned ×2) — tight, **hero-led runtime cards**, now **solid** (`ApplySolidCard`) so the numbers never collide with backdrop art, each ring keyed to its **menu mode token** (DAILY orchid / CLASSIC blue-violet / TIME ATTACK magenta — one colour language across screens): a **DAILY** headline card with the **streak as the hero** (big gold number) and **Longest / Win % / W–L** beside it, a matched **CLASSIC** + **TIME ATTACK** pair of caption-over-value cells, and a slim Overall footer — with the **periwinkle coin pill** (gold coin/number) and a **ghost HOME** (Task 43 tier 3). | <img src="docs/screenshots/stats.png" width="250"> |
...
| **Shop** (Tasks 33, 36; modernized + v1.0-gated) — opened by the **gold coin pill**, which also renders **in the shop header** (the shared pill, not plain text). Every row is a **solid card** (`ApplySolidCard` — rounded near-opaque `SurfaceVoid` fill + accent ring, so prices never fight the backdrop), **Back/Restore are ghosts**, and rows have real internal padding. Content for **v1.0**: **Power-Ups** (Hint / Undo / Reveal / Time in **×5 / ×15 / ×40** tiered coin prices) and a **Free Coins · watch an ad** row (capped 3/day, with a disabled **"Loading…"** state while the rewarded ad loads). The **real-money sections — Starter Pack (gold-ring hero row + filled-hero price), coin packs, Remove Ads, Restore — are HIDDEN behind `ShopScreen.RealMoneyEnabled = false`** until real billing lands (planned 1.1); the UI for them is fully built and returns with a one-line flip. Rebuilds from live state after each buy; unaffordable bundles disable ([§3](#3-economy--monetization)). | <img src="docs/screenshots/shop.png" width="250"> |
...
**The Shop** (`ShopScreen` — a runtime overlay: the shared coin pill in the header, **solid-card rows** via `ApplySolidCard`, ghost Back/Restore) is opened by the **gold coin pill** (`UIManager` → `OnShopRequested` → `GameBootstrap`). **v1.0 ships with real-money commerce HIDDEN** behind `ShopScreen.RealMoneyEnabled = false` (the only `IStoreService` is a mock — selling would be fake purchases): the Starter Pack, coin packs, Remove Ads, and Restore sections are fully built but gated, returning with a one-line flip when real billing lands. What shows in v1.0: the coins-priced **Power-Up** bundles and the **Free Coins · watch an ad** row (disabled **"Loading…"** while the rewarded ad loads, with a bounded readiness poll). It rebuilds from live state after each purchase; unaffordable bundles disable; when enabled, Remove-Ads / Starter-Pack flip to **"Owned."** The **Daily Rewards** popup (`DailyRewardPopup`, same runtime-overlay idiom) is a separate menu surface for the login claim + streak repair, shown when either is available.
...
- **Run (human):** Window → General → Test Runner → **PlayMode** → Run All. **MCP agents:** `run_tests(mode="PlayMode")` — it works (full suite **378/378** as of the v1.0 audit, which added 3 corrupt-save regression tests); EditMode returns 0 (see [§17](#17-notes-for-ai-agents-working-in-this-repo)).
...
- All tests stay green — run via run_tests(mode="PlayMode") (PlayMode-registered suite, 378 tests;
...
3. Tests: Window → General → Test Runner → **PlayMode** → Run All (the suite is PlayMode-registered, **378 tests**).
