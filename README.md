...
| **Puzzle Library → Tier Select** (level 1) — the entry to Puzzle Show: a list of **7 tiers**, each with its theme (e.g. "3-letter words"), progress (`X/100`) and lock state (**gold** = current tier; **padlock + "Clear N in Tier M"** = locked). Tap an unlocked tier to open its grid. | <img src="docs/screenshots/puzzle-library.png" width="250"> |
...
- **Outline buttons + neon glow:** every button is a **colored rounded outline with a transparent center** (not a fill), so the black backdrop shows through. Each action owns a color from the menu set — **Resume** teal, **Daily** orange (hero), **Classic** green, **Puzzle Show** violet, **Time Attack** red, **Library/Stats** muted slate. The hero (Daily) gets a **thicker, brighter** orange ring; light/white labels sit on every button. Each outline carries a **tight, static neon-tube glow** tinted to its own color (`UITheme.ApplyNeonGlow` — a behind-the-button copy of the ring sprite spread as 8 faint `Shadow` samples over a ~2px radius, so the stroke reads as a luminous *line*, not a fuzzy halo; the hero is brighter, never wider). The ring + black treatment is centralized in `UITheme` (`ApplyOutlineButton` / `ApplyHeroOutlineButton`, 9-slice ring sprites under `Assets/Resources/UI/`).
- **Black background + swappable space layer (video or still):** the app renders on a neutral near-black `#0A0A0A`, painted by a single full-screen **Background layer** behind every screen (`UIThemeManager.ApplyScreenBackground` / `EnsureBackgroundLayer`). Backdrop priority is **looping video → still sprite → flat black**, all swappable by just dropping a file in (no scene edit): a muted, looping `Assets/Resources/UI/SpaceBackground.mp4` (driven by a `VideoPlayer` → `RenderTexture` on a full-screen `RawImage`, `EnsureVideoBackground`) wins; else `SpaceBackground.png`; else flat black. A pixel-art space loop ships now.
