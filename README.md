...
- **Grade** — pure `PathScoring.Score(par, steps, detours, mistakesUsed, ranOutOfMistakes, usedPowerUp)` → `PathGrade` where `(int)grade == stars`: **Perfect** (★★★, 0 detours) / **Good** (★★, ≤ `GoodMaxDetours` = 2) / **Solved** (★) / **Failed** (☆, out of mistakes). Surfaced on `ResultsScreen.ShowDailyResult` as a row of **drawn star meshes** (`StarGraphic` — the bundled TMP font has no ★ glyph, so the rating is rendered as geometry, not text) above a gold grade word + "Par N · You got X", inside a subtle ghost result card.
